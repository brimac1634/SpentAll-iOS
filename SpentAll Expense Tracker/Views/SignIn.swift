        ...
        ZStack {
            ...
            VStack {
                ...
                CustomButton(label: "Login", isOn: true, color: Color.spentPink()) {
                    print(self.environmentViewModel.userViewModel.isLoggedIn)
                    self.environmentViewModel.userViewModel.handleLogin(email: self.email, password: self.password)
                }.padding(.vertical, CGFloat(10))
                Text(verbatim: "OR")
                    .font(Font.karla(20))
                    .kerning(2)
...
struct SignIn_Previews: PreviewProvider {
    static var previews: some View {
        SignIn().environmentObject(EnvironmentViewModel())
    }
}
